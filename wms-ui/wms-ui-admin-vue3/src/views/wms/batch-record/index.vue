<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="移动记录ID" prop="movementId">
        <el-input
          v-model="queryParams.movementId"
          placeholder="请输入移动记录ID"
          clearable
          @keyup.enter="handleQuery"
          class="!w-240px"
        />
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['wms:batch-record:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['wms:batch-record:export']"
        >
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="移动记录ID" align="center" prop="movementId" />
      <el-table-column label="操作数量" align="center" prop="count" />
      <el-table-column label="操作前数量" align="center" prop="beforeCount" />
      <el-table-column label="操作后数量" align="center" prop="afterCount" />
      ...
      <el-table-column label="操作" align="center" min-width="120px">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['wms:batch-record:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['wms:batch-record:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <BatchRecordForm ref="formRef" @success="getList" />
</template>
...
const list = ref<BatchRecordVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BatchRecordApi.getBatchRecordPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await BatchRecordApi.deleteBatchRecord(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await BatchRecordApi.exportBatchRecord(queryParams)
    download.excel(data, '批次操作记录.xls')
  } catch {
  } finally {
