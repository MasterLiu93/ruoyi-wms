...
CREATE TABLE `wms_location` (
                                ...
                                `area_id` bigint(20) NOT NULL COMMENT '货区ID',
...
CREATE TABLE `wms_receipt_order_detail` (
                                            ...
                                            `area_id` bigint(20) DEFAULT NULL COMMENT '入库货区ID',
                                            `rack_id` bigint(20) DEFAULT NULL COMMENT '入库货架ID',
...
CREATE TABLE `wms_receipt_record` (
                                      ...
                                      `tenant_id` bigint(20) NOT NULL DEFAULT '0' COMMENT '租户编号',
                                      PRIMARY KEY (`id`),
                                      INDEX `idx_receipt_order_id` (`receipt_order_id`),
                                      INDEX `idx_receipt_order_detail_id` (`receipt_order_detail_id`)
...
VALUES
    ...
    (5, '已取消', 4, 'wms_order_status', 0, '已取消', 'admin', now(), 'admin', now(), b'0');
...
-- 添加二级菜单：入库管理
INSERT INTO `system_menu` (`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `status`, `visible`, `keep_alive`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
VALUES ('入库管理', '', 1, 20, @parentId, 'receipt', 'ep:goods-filled', NULL, 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0');

-- 获取入库管理的菜单ID
SET @receiptId = (SELECT `id` FROM `system_menu` WHERE `parent_id` = @parentId AND `name` = '入库管理' LIMIT 1);

-- 入库单管理
INSERT INTO `system_menu` (`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `status`, `visible`, `keep_alive`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
VALUES ('入库单管理', 'wms:receipt-order:list', 2, 10, @receiptId, 'receipt-order', 'ep:document', 'wms/receipt-order/index', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0');

SET @receiptOrderId = (SELECT `id` FROM `system_menu` WHERE `parent_id` = @receiptId AND `name` = '入库单管理' LIMIT 1);

INSERT INTO `system_menu` (`name`, `permission`, `type`, `sort`, `parent_id`, `path`, `icon`, `component`, `status`, `visible`, `keep_alive`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
VALUES
    ('入库单查询', 'wms:receipt-order:query', 3, 1, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0'),
    ('入库单创建', 'wms:receipt-order:create', 3, 2, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0'),
    ('入库单更新', 'wms:receipt-order:update', 3, 3, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0'),
    ('入库单删除', 'wms:receipt-order:delete', 3, 4, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0'),
    ('入库单导出', 'wms:receipt-order:export', 3, 5, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0'),
    ('入库单审核', 'wms:receipt-order:approve', 3, 6, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0'),
    ('执行入库', 'wms:receipt-record:create', 3, 7, @receiptOrderId, '', '', '', 0, b'1', b'1', 'admin', now(), 'admin', now(), b'0');
...
-- ----------------------------
-- 修改入库单明细表，添加批次ID字段（如果不存在）
-- ----------------------------
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_receipt_order_detail'
            AND COLUMN_NAME = 'batch_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_receipt_order_detail` ADD COLUMN `batch_id` bigint(20) DEFAULT NULL COMMENT \'批次ID\' AFTER `location_id`', 'SELECT \'Column batch_id exists in wms_receipt_order_detail\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ----------------------------
-- 修改出库单明细表，添加批次ID字段（如果不存在）
-- ----------------------------
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_shipment_order_detail'
            AND COLUMN_NAME = 'batch_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_shipment_order_detail` ADD COLUMN `batch_id` bigint(20) DEFAULT NULL COMMENT \'批次ID\' AFTER `location_id`', 'SELECT \'Column batch_id exists in wms_shipment_order_detail\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ----------------------------
-- 修改入库操作记录表，添加批次ID字段（如果不存在）
-- ----------------------------
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_receipt_record'
            AND COLUMN_NAME = 'batch_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_receipt_record` ADD COLUMN `batch_id` bigint(20) DEFAULT NULL COMMENT \'批次ID\' AFTER `location_id`', 'SELECT \'Column batch_id exists in wms_receipt_record\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ----------------------------
-- 修改出库操作记录表，添加批次ID字段（如果不存在）
-- ----------------------------
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_shipment_record'
            AND COLUMN_NAME = 'batch_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_shipment_record` ADD COLUMN `batch_id` bigint(20) DEFAULT NULL COMMENT \'批次ID\' AFTER `location_id`', 'SELECT \'Column batch_id exists in wms_shipment_record\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- ----------------------------
-- 修改移库明细表和记录表，添加批次ID字段（如果不存在）
-- ----------------------------
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_move_order_detail'
            AND COLUMN_NAME = 'batch_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_move_order_detail` ADD COLUMN `batch_id` bigint(20) DEFAULT NULL COMMENT \'批次ID\' AFTER `item_id`', 'SELECT \'Column batch_id exists in wms_move_order_detail\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_move_record'
            AND COLUMN_NAME = 'batch_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_move_record` ADD COLUMN `batch_id` bigint(20) DEFAULT NULL COMMENT \'批次ID\' AFTER `item_id`', 'SELECT \'Column batch_id exists in wms_move_record\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 确保入库单表结构完整（如果列不存在则添加）
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_receipt_order'
            AND COLUMN_NAME = 'completion_time');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_receipt_order` ADD COLUMN `completion_time` datetime DEFAULT NULL COMMENT \'完成时间\' AFTER `arrival_time`', 'SELECT \'Column completion_time exists in wms_receipt_order\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 确保入库单状态字典包含"已取消"状态
INSERT INTO `system_dict_data` (`sort`, `label`, `value`, `dict_type`, `status`, `remark`, `creator`, `create_time`, `updater`, `update_time`, `deleted`)
SELECT 5, '已取消', 4, 'wms_order_status', 0, '已取消', 'admin', now(), 'admin', now(), b'0'
FROM dual
WHERE NOT EXISTS (
    SELECT 1 FROM `system_dict_data` 
    WHERE `dict_type` = 'wms_order_status' AND `value` = '4'
);

-- 更新已完成入库单的状态
UPDATE `wms_receipt_order` ro
SET ro.`receipt_status` = 2, 
    ro.`completion_time` = IFNULL(ro.`completion_time`, NOW())
WHERE ro.`order_status` = 2  -- 审核通过
  AND ro.`receipt_status` <> 2  -- 不是已完成状态
  AND NOT EXISTS (
      SELECT 1 FROM `wms_receipt_order_detail` rod
      WHERE rod.`receipt_order_id` = ro.`id`
        AND rod.`deleted` = 0
        AND (rod.`status` <> 2 OR rod.`real_count` < rod.`plan_count`)
  );

-- 更新部分入库单的状态
UPDATE `wms_receipt_order` ro
SET ro.`receipt_status` = 1  -- 部分入库
WHERE ro.`order_status` = 2  -- 审核通过
  AND ro.`receipt_status` = 0  -- 目前是待入库状态
  AND EXISTS (
      SELECT 1 FROM `wms_receipt_order_detail` rod
      WHERE rod.`receipt_order_id` = ro.`id`
        AND rod.`deleted` = 0
        AND rod.`real_count` > 0  -- 有实际入库数量
  );

-- 添加索引，如果不存在的话
-- 检查wms_receipt_record表的idx_receipt_order_id索引是否存在
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_receipt_record'
            AND INDEX_NAME = 'idx_receipt_order_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_receipt_record` ADD INDEX `idx_receipt_order_id` (`receipt_order_id`)', 'SELECT \'Index idx_receipt_order_id already exists on wms_receipt_record\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 检查wms_receipt_record表的idx_receipt_order_detail_id索引是否存在
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_receipt_record'
            AND INDEX_NAME = 'idx_receipt_order_detail_id');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_receipt_record` ADD INDEX `idx_receipt_order_detail_id` (`receipt_order_detail_id`)', 'SELECT \'Index idx_receipt_order_detail_id already exists on wms_receipt_record\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 检查wms_inventory表的idx_warehouse_item索引是否存在
SET @s = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = 'wms_inventory'
            AND INDEX_NAME = 'idx_warehouse_item');

SET @sql = IF(@s = 0, 'ALTER TABLE `wms_inventory` ADD INDEX `idx_warehouse_item` (`warehouse_id`, `item_id`)', 'SELECT \'Index idx_warehouse_item already exists on wms_inventory\'');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
